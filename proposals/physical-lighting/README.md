...
This consists of two schema: `PhysicalCameraResponsivityAPI` and `PhysicalCameraQuantumEfficiencyAPI`, which both describe the same concept but with different parameterizations that are suited to different use cases.
...
## `PhysicalCameraQuantumEfficiencyAPI`

`PhysicalCameraQuantumEfficiencyAPI` is intended for camera sensors that are generating raw images and is parameterized by N functions, one for each of the N colour filters in the sensor's matrix, describing the ratio of electrons generated by the sensor for each photon arriving at a given wavelength. The API also specifies the pattern of colour filters in the sensor matrix, e.g. "RGGB"
...
class PhysicalCameraQuantumEfficiencyAPI (
    customData = {
        token apiSchemaType = "singleApply"
        token[] apiSchemaCanOnlyApplyTo = ["Camera"]
    }
...
{
    string physical:colorFilterArrayPattern = "RGGB" (
        doc = """The pattern of the sensor's CFA"""
        displayName = "Color Filter Array Pattern"
    )

    # TODO: how do we specify an unknown number of curves here?
}
