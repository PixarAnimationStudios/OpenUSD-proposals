...
To start, it is useful to define exactly what we mean by namespace editing. In the context of this proposal, a namespace edit is an operation that removes or changes the namespace path of a **composed object** on a stage. On a UsdStage, these composed objects consist of prims and properties with a property being either an attribute or a relationship. The types of operations that are considered namespace edits are deleting, renaming, and reparenting a composed prim or property.
